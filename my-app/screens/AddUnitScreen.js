...
const AddUnitScreen = ({ navigation }) => {
  ...
  const handleSave = async () => {
    if (!name.trim()) return Alert.alert('กรุณากรอกชื่อหน่วยยา');
    const userId = await AsyncStorage.getItem('userId');
    try {
      const res = await fetch(`${BASE_URL}/api/units`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ DosageType: name, UserID: userId ? parseInt(userId, 10) : null }),
      });
      if (res.ok) {
        const json = await res.json();
        const newId = json.id ?? null;
        Alert.alert('บันทึกเรียบร้อย');
        navigation.navigate('AddMedication', { newUnitId: newId, newUnitName: name });
      } else {
        const txt = await res.text().catch(() => '');
        console.error('create unit failed', txt);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.label}>ชื่อหน่วยยา</Text>
