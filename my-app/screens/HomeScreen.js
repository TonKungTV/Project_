...
// // ✅ Component สำหรับแสดงสถานะ (กินแล้ว, ยังไม่กิน, ยังไม่บันทึก)
// const StatusBadge = ({ status, onPress }) => {
//   const getStatusConfig = (status) => {
//     switch (status) {
//       case 'กินแล้ว':
...
//         return { color: '#dc3545', icon: 'close-circle', text: 'ยังไม่กิน' };
//       case 'ยังไม่บันทึก':
//         return { color: '#ffc107', icon: 'time', text: 'ยังไม่บันทึก' };
//       default:
//         return { color: '#6c757d', icon: 'time', text: 'ยังไม่บันทึก' };
//     }
//   };
...
//     { key: 'ยังไม่บันทึก', label: 'ยังไม่บันทึก', icon: 'time' },
//     { key: 'กินแล้ว', label: 'กินแล้ว', icon: 'checkmark-circle' },
//     { key: 'ยังไม่กิน', label: 'ยังไม่กิน', icon: 'close-circle' }
//   ];
...
//               name={filter.icon} 
//               size={16} 
//               color={activeFilter === filter.key ? '#fff' : '#4dabf7'} 
//             />
//             <Text style={[
...
//   // ✅ ฟังก์ชันอัพเดทสถานะ
//   const updateMedicationStatus = async (item, newStatus, customSideEffects = '', customTime = '') => {
...
//   // ✅ ฟังก์ชันสำหรับปุ่มต่างๆ ใน Modal
//   const handleTaken = () => {
//     if (selectedItem) {
//       updateMedicationStatus(selectedItem, 'กินแล้ว', sideEffects, actualTakeTime);
//       Alert.alert(
//         'บันทึกสำเร็จ', 
//         `บันทึกการกินยา "${selectedItem.name}" เรียบร้อยแล้ว${sideEffects ? '\nรวมถึงผลข้างเคียง' : ''}`,
//         [{ text: 'ตกลง' }]
...
//     if (selectedItem) {
//       updateMedicationStatus(selectedItem, 'ยังไม่กิน', sideEffects, actualTakeTime);
//       Alert.alert('บันทึกสำเร็จ', `บันทึกว่ายังไม่ได้กินยา "${selectedItem.name}"`);
//     }
//     closeModal();
//   };
...
//   const stats = {
//     total: items.length,
//     taken: items.filter(item => item.status === 'กินแล้ว').length,
//     notTaken: items.filter(item => item.status === 'ยังไม่กิน').length,
//     notRecorded: items.filter(item => item.status === 'ยังไม่บันทึก').length
//   };
...
//               {stats.notRecorded}
//             </Text>
//             <Text style={styles.summaryLabel}>ยังไม่บันทึก</Text>
//           </View>
//         </View>
//       </View>
...
//       {/* ✅ Modal สำหรับยืนยันการกินยา - ปรับปรุงปุ่มใหม่ */}
//       <Modal visible={modalVisible} animationType="slide" transparent={true}>
//         <View style={styles.modalBackground}>
//           <View style={styles.modalContent}>
//             <View style={styles.modalHeader}>
...
//                 {/* ✅ ปุ่มใหม่: ทานยาแล้ว, ยังไม่ทาน, ยกเลิก */}
//                 <View style={styles.modalButtonContainer}>
//                   <TouchableOpacity style={styles.takenBtn} onPress={handleTaken}>
//                     <Ionicons name="checkmark-circle" size={18} color="#fff" />
//                     <Text style={styles.buttonText}>ทานยาแล้ว</Text>
//                   </TouchableOpacity>
                  
//                   <TouchableOpacity style={styles.notTakenBtn} onPress={handleNotTaken}>
//                     <Ionicons name="close-circle" size={18} color="#fff" />
//                     <Text style={styles.buttonText}>ยังไม่ทาน</Text>
//                   </TouchableOpacity>
